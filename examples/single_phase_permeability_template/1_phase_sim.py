...
def download_geometry(filename, url):
    ...
    download_command = f"wget {url} --continue -O {filename}"
    try:
        subprocess.run(download_command.split(" "))
    except FileNotFoundError:
        raise InterruptedError(
            "wget was not found. Please make sure it is installed on your system."
        )
    return
...
def run_1_phase_sim(inputs):
    ...
    file.close()
    simulation_command_subproc = (
        f"bash {sim_directory}/{input_dir}run_single_phase_sim.sh"
    )
